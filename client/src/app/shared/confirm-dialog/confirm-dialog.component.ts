  ...
  template: `
    <app-modal [isOpen]="isOpen" [title]="title" [showFooter]="true" (closed)="onCancel()">
      ...
      <ng-container modal-footer>
        <button type="button" class="btn btn-outline-secondary" (click)="onCancel()"><span class="material-symbols-outlined">close</span>{{ cancelLabel }}</button>
        <button type="button" class="btn btn-primary" (click)="onConfirm()"><span class="material-symbols-outlined">check</span>{{ confirmLabel }}</button>
      </ng-container>
    </app-modal>
  `
})
